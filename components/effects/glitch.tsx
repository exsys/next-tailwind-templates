...
/**
Tailwind: Add the following to keyframes:
    glitch: {
...
And the following to animation:
...
export default function Glitch({ src, width, height, style }: Props) {
    const divHeight = height ? `${height}px` : "100%";
    const divWidth = width ? `${width}px` : "100%";

    return (
        <div className={`relative`} style={{ width: divWidth, height: divHeight }}>
            <div className={`overflow-hidden absolute z-10 animate-glitch ${style}`}>
                <img src={src} width={divWidth} height={divHeight} alt={src} />
            </div>
            <div className={`absolute -bottom-[23px] -right-[25px] z-0 overflow-hidden animate-glitchImg1 ${style}`}>
                <img src={src} width={divWidth} height={divHeight} alt={src} />
            </div>
            <div className={`absolute -top-[23px] -left-[25px] overflow-hidden animate-glitchImg2 ${style}`}>
                <img src={src} width={divWidth} height={divHeight} alt={src} />
            </div>
        </div>
    )
